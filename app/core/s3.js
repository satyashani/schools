...
var AWS = require("./AWS"); 
var conf = require("config").get("aws");
var s3 = new AWS.S3(); 
...
exports.listRawDataFiles = function(username,callback){
    listContent(conf.bucket,conf.rawData+"/"+username+"/",callback);
};

exports.deleteRawDataFile = function(filename,callback){
    remove(conf.bucket,conf.rawData+"/"+filename,callback);
};

exports.getRawDataFileInfo = function(filename,callback){
    hasObject(conf.bucket,conf.rawData+"/"+filename,callback);
};

exports.getRawDataFile = function(filename,callback){
    getFile(conf.bucket,conf.rawData+"/"+filename,callback);
};

exports.syncRawData = function(sourceBucket,sourceUsername,destUsername,callback){
    sync(sourceBucket,conf.rawData+"/"+sourceUsername,conf.bucket,conf.rawData+"/"+destUsername,callback);
};
